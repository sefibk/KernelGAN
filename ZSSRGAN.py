...
class ZSSRGAN:
    ...
    def epoch_D_for_ZSSR(self, d_input):
        loss_d = self.calc_loss_d(d_input)
        # Calculate gradients, note that gradients are not propagating back through generator
        loss_d.backward()
        # Update weights, note that only discriminator weights are updated (by definition of the D optimizer)
        self.optimizer_D.step()

    def calc_loss_d(self, d_input=None):
        # Zeroize gradients
        ...
        if d_input is None:
            ...
            d_pred_real = self.D.forward(im2tensor(d_input))
            # Discriminator forward pass over fake example (generated by generator)
            # Note that generator result is detached so that gradients are not propagating back through generator
        ...
        loss_d_fake = self.criterionGAN(d_pred_fake, is_d_input_real=False, zssr_shape=d_input is not None)
        loss_d_real = self.criterionGAN(d_pred_real, is_d_input_real=True, zssr_shape=d_input is not None)
